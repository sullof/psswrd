...
class Secret {
  ...
  init(json) {
    if (json.id) {
      return Promise.resolve(this.fromJSON(json))
    } else {
      this.key = Crypto.getRandomString(64)
      this.name = json.name
      this.tags = json.t
      this.version = 0
      this.db.newId()
          .then(id => Promise.resolve(this.id = id))
    }
  }
  ...
  getVersionedFilename() {
    const [key, salt] = this.splitKey()
    return Crypto.toSHA256(salt + this.version, key, 'base64')
        .then(filename => path.join(this.id, filename))
  }
